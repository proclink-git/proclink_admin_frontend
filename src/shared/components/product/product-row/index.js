...
function ProductRow({ data, index, selected, onDelete, onStatusChange, onSelect, bulkPermission, eStatus }) {
  return (
    <tr key={data?._id}>
      ...
      <td>
        <PermissionProvider isAllowedTo={PRODUCT_UPDATE_PERMISSIONS} isArray>
        ...
        <PermissionProvider isAllowedTo={PRODUCT_EDIT_PERMISSIONS} isArray>
          <ToolTip toolTipMessage={<FormattedMessage id="edit" />}>
            <Button variant="link" className="square icon-btn" as={Link} to={allRoutes.editProduct(data?._id)}>
