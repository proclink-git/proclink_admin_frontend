...
function getDefaultIndustryPage() {
  return {
    oIHC: {
      sTitle: '',
      sDescription: '',
      sImageTitle: '',
      oImg: getDefaultImage(),
      aCard: [getDefaultIndustryHeroCard()]
    },
    oIST: {
      sTitle: '',
      sDescription: '',
      aCard: [getDefaultIndustryStackCard()]
    },
    oCB: {
      sTitle: '',
      sDescription: '',
...
function normalizeIndustryHeroCard(card = {}) {
  return {
...
function normalizeIndustryStackCard(card = {}) {
  return {
...
function normalizeIndustryPage(industryPage = {}) {
  const data = removeTypenameKey(industryPage) || {}

  return normalizePageNamespaceComponents({
    ...getDefaultIndustryPage(),
    ...data,
    oIHC: {
      sTitle: data?.oIHC?.sTitle || '',
      sDescription: data?.oIHC?.sDescription || '',
      sImageTitle: data?.oIHC?.sImageTitle || '',
      oImg: normalizeImage(data?.oIHC?.oImg),
      aCard: normalizeArray(data?.oIHC?.aCard, getDefaultIndustryHeroCard, normalizeIndustryHeroCard)
    },
    oIST: {
      sTitle: data?.oIST?.sTitle || '',
      sDescription: data?.oIST?.sDescription || '',
      aCard: normalizeArray(data?.oIST?.aCard, getDefaultIndustryStackCard, normalizeIndustryStackCard)
    },
    oCB: {
...
function normalizeIndustryPageForSubmit(industryPage = {}) {
  const data = removeTypenameKey(industryPage) || {}

  const cleanCtas = (items = [], limit) => {
    const nextItems = (Array.isArray(items) ? items : [])
      .map((item) => ({
        sLabel: item?.sLabel || '',
        sUrl: item?.sUrl || ''
  ...
  return {
    ...
    oIHC: {
      ...
      sImageTitle: data?.oIHC?.sImageTitle || '',
      oImg: normalizeImage(data?.oIHC?.oImg),
      aCard: (Array.isArray(data?.oIHC?.aCard) ? data.oIHC.aCard : [])
        .map(normalizeIndustryHeroCard)
        .filter((card) => card?.sTitle || card?.sDescription || card?.sRedirectUrl || card?.oImg?.sUrl)
    },
    oIST: {
      sTitle: data?.oIST?.sTitle || '',
      sDescription: data?.oIST?.sDescription || '',
      aCard: (Array.isArray(data?.oIST?.aCard) ? data.oIST.aCard : [])
        .map(normalizeIndustryStackCard)
        .filter((card) => card?.sTitle || card?.sDescription || card?.sRedirectUrl || card?.sButtonText || card?.oImg?.sUrl)
    },
    oCB: {
      ...data?.oCB,
      oImg: normalizeImage(data?.oCB?.oImg),
...
const editorSectionPrefix = 'industry-page-editor'

function IndustryPage({ pageData }) {
  ...
  const selectedComponents = watch('oIndustryPage.aComponent') || []
  ...
  function getFormValue() {
    return {
      ...
      ePageType: pageData?.ePageType || 'i',
      oImg: pageData?.oImg ? removeTypeName(pageData?.oImg) : getDefaultImage(),
      oIndustryPage: normalizeIndustryPage(pageData?.oIndustryPage),
      oSeo: {
  ...
  function onSubmit(data) {
    const duplicateDataKeys = findDuplicatePageComponentDataKeys(data?.oIndustryPage?.aComponent, data?.ePageType || 'i')
    if (duplicateDataKeys.length) {
      setError('oIndustryPage.aComponent', {
        type: 'validate',
    ...
    const nextData = {
      ...data,
      oIndustryPage: buildPageNamespacePayload(normalizeIndustryPageForSubmit(data?.oIndustryPage), data?.ePageType || 'i')
    }
  ...
  return (
    <FormProvider {...methods}>
      <Form className="page-component-editor-form page-component-editor-form--has-sticky-picker" onSubmit={handleSubmit(onSubmit)}>
        <AddPageComponents
          control={control}
          error={errors?.oIndustryPage?.aComponent}
          name="oIndustryPage.aComponent"
          ePageComponentType={pageData?.ePageType || 'i'}
        ...
        <Row>
          <Col lg="9">
            ...
            <AddPageComponentsList name="oIndustryPage.aComponent" namespace="oIndustryPage" pageType={pageData?.ePageType || 'i'} anchorPrefix={editorSectionPrefix} />
