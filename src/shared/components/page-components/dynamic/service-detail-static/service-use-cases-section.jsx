...
import { getDefaultServiceUseCaseCard } from './utils'
...
export default function ServiceUseCasesSection({ basePath = 'oSUC' }) {
  const {
    register,
    control,
    watch,
  ...
  useEnsureFieldArrayDefault({ fields, append, getValues, name: fieldArrayName, getDefaultValue: getDefaultServiceUseCaseCard })

  return (
    <div className="p-3">
      <Form.Label className="text-uppercase small text-muted mb-3">Service Use Cases</Form.Label>
      ...
      <Form.Label className="text-uppercase small text-muted mb-2">Use Case Cards</Form.Label>
          ...
          actions={
            <>
              {index + 1 === fields.length && (
                <Button type="button" onClick={() => append(getDefaultServiceUseCaseCard())} variant="link" size="sm" className="square icon-btn">
        ...
        >
          ...
          <Row>
            <Col md="4" className="add-article">
              <ImageDimensionNote width={824} height={476} subject="image" />
...
ServiceUseCasesSection.propTypes = {
