...
function ServiceRow({ data, index, selected, onDelete, onStatusChange, onSelect, bulkPermission, eStatus }) {
  return (
    <tr key={data?._id}>
      ...
      <td>
        <PermissionProvider isAllowedTo="UPDATE_SERVICE_STATUS">
        ...
        <PermissionProvider isAllowedTo="EDIT_SERVICE">
          <ToolTip toolTipMessage={<FormattedMessage id="edit" />}>
            <Button variant="link" className="square icon-btn" as={Link} to={allRoutes.servicesEdit(data?._id)}>
