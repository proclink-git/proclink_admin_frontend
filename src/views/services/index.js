...
import { LIST_SERVICES } from 'graph-ql/services/query'
import ServiceRow from 'shared/components/service/service-row'
import { BULK_OPERATION_SERVICES } from 'graph-ql/services/mutation'

function Services({ userPermission }) {
  ...
  const bulkActionDropDown = [
    { label: 'Delete All', value: 'd', isAllowedTo: 'UPDATE_SERVICE_STATUS' },
    { label: 'Active  All', value: 'a', isAllowedTo: 'UPDATE_SERVICE_STATUS' },
    { label: 'Deactivate All', value: 'i', isAllowedTo: 'UPDATE_SERVICE_STATUS' }
  ]
  ...
  const { loading, refetch } = useQuery(LIST_SERVICES, {
  ...
  const [bulkAction, { loading: bulkLoading }] = useMutation(BULK_OPERATION_SERVICES, {
    onCompleted: (data) => {
      if (data && data.bulkServiceUpdate) {
        dispatch({
          type: 'SHOW_TOAST',
          payload: { message: data.bulkServiceUpdate?.sMessage, type: TOAST_TYPE.Success, btnTxt: labels.close }
        })
      }
    }
  })
  ...
  function handleApiResponse(data) {
    if (data && data?.listService?.aResults) {
      setSelectedCategory(
        data.listService?.aResults.map((item) => {
          return {
            _id: item._id,
            value: false
          }
      ...
      totalRecord.current = data.listService.nTotal
      setCategoryList(data.listService?.aResults)
    }
  }
  ...
  function handleBtnEvent(eventName) {
    switch (eventName) {
      case 'newService':
        history.push(allRoutes.createService)
        break
      default:
        break
    }
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            ...
            if (data && data.bulkServiceUpdate) handleBulkResponse([id], 'd')
          }
        },
        {
          label: labels.no
  ...
  async function handleStatusChange({ target }) {
    ...
    if (data?.bulkServiceUpdate) handleBulkResponse([target.name], target.checked ? 'a' : 'i')
  }
  ...
  return (
    <>
      <TopBar
        buttons={[{ text: 'New Service', icon: 'icon-add', type: 'primary', clickEventName: 'newService', isAllowedTo: 'CREATE_SERVICE' }]}
